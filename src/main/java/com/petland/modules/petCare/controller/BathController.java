...
public class BathController {
    ...
    @GetMapping
    public ResponseEntity<Page<BathResponseDTO>> findAllBathServices(@RequestParam(required = false, defaultValue = "0") int page,
                                                                     @RequestParam(required = false, defaultValue = "10") int size){
        Page<BathResponseDTO> bathsServicesList = bathService.listAllBathServices(PageRequest.of(page, size));
        return ResponseEntity.ok(bathsServicesList);
    }
}
